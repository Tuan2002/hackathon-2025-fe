  ...
  return (
    <Card className='overflow-hidden shadow-md transition hover:shadow-lg bg-white dark:bg-gray-800'>
      ...
      <div className='p-2 space-y-2'>
        <h3 className='text-sm font-semibold text-gray-800 dark:text-gray-100 line-clamp-2 h-20 text-justify'>
          {document.name}
        </h3>
        ...
        <div className='flex items-center justify-between mt-2 text-sm text-gray-500 dark:text-gray-400'>
          <span className='flex items-center gap-1'>
            <Eye size={14} /> {document.viewCount}
          </span>
          <span className='flex items-center gap-1'>
            <Download size={14} /> {document.downloadCount}
          </span>
        </div>

        <Link
          to={`/document-detail/${document.slug}`}
